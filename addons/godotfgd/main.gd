...
func get_flags(script: Script):
	var index = 0;
	var flags = script.get_script_constant_map().keys().filter(func(n: String): return n.begins_with("FLAG_"));
...
func get_prop_description(script: Script, property_name: String, no_decorators = true):
	...
	var description = "";
	var line_index = get_property_line_index(script, property_name);
	if line_index == -1: return get_prop_description(script.get_base_script(), property_name);

	var comment_line = source[line_index - 1];
	line_index -= 1;
